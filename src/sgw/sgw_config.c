  ...
  if (setting_sgw) {
    ...
    if (subsetting) {
      ...
      if (config_pP->is_remote_controller_enabled && config_setting_lookup_string (subsetting, SGW_CONFIG_STRING_SGW_REMOTE_CONTROLLER_IPV4_ADDRESS, (const char **)&remote_controller)) {
        ...
        IPV4_STR_ADDR_TO_INT_NWBO(bdata(cidr), config_pP->ipv4.remote_controller, "BAD IP ADDRESS FORMAT FOR REMOTE CONTROLLER !\n");
        in_addr_var.s_addr = config_pP->ipv4.remote_controller;
